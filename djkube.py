...
def home(name=None):
    ...
    if len(costexplorer.reports[0]['Data']) > 1:
        if context['previous_month_cost'] > 0:
...
def init_git_repo(github_api_token, github_repo_name, download_file_name,aws_region,docker_registry_name):
    try:
        ...
        repo.create_file(".github/workflows/main.yaml", "enable workflow file", decoded_content, branch="master")
        ...
        # repo.delete_file(contents.path, "remove old file", contents.sha, branch="master")
        repo.create_file(".github/workflows/main.yaml" , "enable workflow file", decoded_content, branch="feature")
        # repo.update_file(contents.path,  "change name of target branch", modified_content,contents.sha, branch="master")
...
def deploy_in_development(cluster_id):
    ...
    aws_region = model.get_cluster(cluster_id).region
    output = commander.execute_command(f'aws ecr list-images --region {aws_region} --repository-name {app_cnf.docker_registry_name}')
    ...
    # Get ECR URI
    aws_region = model.get_cluster(cluster_id).region
    output = commander.execute_command(f'aws ecr describe-repositories --region {aws_region} --repository-name {app_cnf.docker_registry_name}')
...
if __name__ == "__main__":
    ...
    logo = '''
    -------------------------------------------------------------------------------
    |           Welcome to "djkube" - Your Full Stack Kubernetes Enabler          |  
    |                                                                             |
